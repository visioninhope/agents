...
const getNangoClient = () => {
	...
	if (!secretKey) {
		throw new NangoError(
			"NANGO_SECRET_KEY environment variable is required for Nango integration",
		);
	}
	...
	try {
		...
		throw new NangoError(
			"Failed to initialize Nango client",
			"new Nango",
			error,
		);
	}
...
export async function fetchNangoProviders(): Promise<ApiProvider[]> {
	try {
		...
		wrapNangoError(
			error,
			"Unable to retrieve available providers from Nango",
			"listProviders",
		);
	}
}
 ...
 */
export async function fetchNangoProvider(
	providerName: string,
): Promise<ApiProvider> {
	try {
		const nango = getNangoClient();
		...
		wrapNangoError(
			error,
			`Provider '${providerName}' not found or inaccessible`,
			"getProvider",
		);
	}
}
 ...
 */
export async function fetchNangoIntegrations(): Promise<
	ApiPublicIntegration[]
> {
	try {
		const nango = getNangoClient();
		...
		wrapNangoError(
			error,
			"Unable to retrieve existing integrations",
			"listIntegrations",
		);
	}
}
...
export async function fetchNangoIntegration(
	...
	try {
		...
		const response = await nango.getIntegration(
			{ uniqueKey },
			{ include: ["credentials"] },
		);
		...
		const { credentials: _credentials, ...integrationWithoutCredentials } =
			integration;
		...
		wrapNangoError(
			error,
			`Unable to fetch integration with key '${uniqueKey}'`,
			"getIntegration",
		);
	}
}
...
export async function createNangoApiKeyConnection({
	...
	try {
		...
		try {
			...
			if (
				...
				if (existingIntegration) {
					...
					wrapNangoError(
						error,
						`Integration creation failed for unexpected reasons`,
						"create",
					);
				}
			} else {
				...
				wrapNangoError(
					error,
					`Failed to create or retrieve integration '${idFromName}'`,
					"create",
				);
			}
		}
		...
		try {
			...
			wrapNangoError(
				error,
				`Failed to import API key connection to Nango`,
				"importConnection",
			);
		}
		...
		try {
			...
			console.warn(
				`Failed to set metadata for connection ${idFromName}:`,
				error,
			);
		}
		return { integration };
...
export async function createProviderConnectSession({
	...
	try {
		...
		let existingIntegration:
			| (ApiPublicIntegration & { areCredentialsSet: boolean })
			| null = null;
		...
		try {
			...
			console.debug(
				`Integration '${providerName}' not found, will create new one`,
			);
		}
