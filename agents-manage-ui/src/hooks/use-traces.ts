		...
		async (page: number = currentPage) => {
			try {
				...
				const currentStartTime =
					options?.startTime || new Date("2020-01-01T00:00:00Z").getTime();

				const paginationParams = paginationEnabled
					? { page, limit: pageSize }
					: undefined;
...
export function useAggregateStats(options?: {
	...
	const fetchAggregateStats = useCallback(async () => {
		try {
			...
			const errorMessage =
				err instanceof Error ? err.message : "Failed to fetch aggregate stats";
			setError(errorMessage);
		} finally {
			setLoading(false);
		}
