  ...
  const openNangoConnect = useCallback(
    ({ sessionToken, onEvent, connectOptions }: OpenNangoConnectOptions): NangoConnectInstance => {
      ...
      const connect = nango.openConnectUI({
        baseURL:
          connectOptions?.baseURL || process.env.NEXT_PUBLIC_NANGO_CONNECT_BASE_URL || undefined,
        apiURL: connectOptions?.apiURL || process.env.NEXT_PUBLIC_NANGO_HOST || undefined,
        onEvent,
