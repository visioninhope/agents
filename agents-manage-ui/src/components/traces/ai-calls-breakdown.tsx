...
} from "@/hooks/use-ai-calls-query-state";
...
export function AICallsBreakdown({ onBack }: AICallsBreakdownProps) {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const [agentData, modelData, uniqueGraphs, uniqueModels] =
					await Promise.all([
						...
						client.getUniqueGraphs(
							startTime,
							endTime,
							params.projectId as string,
						),
						client.getUniqueModels(
							startTime,
							endTime,
							params.projectId as string,
						),
				...
				setError(
					err instanceof Error ? err.message : "Failed to fetch AI calls data",
				);
			} finally {
				setLoading(false);
			}
	...
	const totalAICalls = agentCalls.reduce(
					...
					{loading ? (
						...
						<div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
							{modelCalls
								.filter(
									(model) =>
										selectedModel === "all" || model.modelId === selectedModel,
								)
								.map((model, index) => (
									<div
										key={index}
