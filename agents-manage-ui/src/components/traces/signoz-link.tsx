...
const SIGNOZ_BASE =
	process.env.NEXT_PUBLIC_SIGNOZ_URL ?? "http://localhost:3080";
...
export function SignozLink({ conversationId }: SignozLinkProps) {
	return (
		<ExternalLink href={makeTracesUrl(conversationId)}>
			View in SigNoz
		</ExternalLink>
	);
}
...
export function SignozSpanLink({ traceId, spanId }: SignozSpanLinkProps) {
	return (
		<ExternalLink href={makeSpanUrl(traceId, spanId)}>
			View in SigNoz
		</ExternalLink>
	);
}
...
export function ConversationTracesLink({
	conversationId,
}: ConversationTracesLinkProps) {
