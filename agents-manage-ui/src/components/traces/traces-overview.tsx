...
import {
	type TimeRange,
	useTracesQueryState,
} from "@/hooks/use-traces-query-state";
import {
...
export function TracesOverview({ refreshKey }: TracesOverviewProps) {
	...
	useEffect(() => {
		const fetchAICallsByGraph = async () => {
			try {
				...
				const aiCallsData = await client.getAICallsByGraph(
					startTime,
					endTime,
					projectId as string,
				);
	...
	const selectedGraphAICalls = useMemo(() => {
		...
		const graphAICalls = aiCallsByGraph.find(
			(ac) => ac.graphId === selectedGraph,
		);
				...
				<div className="flex flex-col md:flex-row gap-2 md:gap-0">
					...
					<div className="space-y-1">
						...
						<div className="flex items-center gap-2">
							<Button
								variant={
									showSpanFilters || spanName || attributes.length > 0
										? "default"
										: "outline"
								}
								size="sm"
					...
					<div className="space-y-4">
						...
						<div className="space-y-1">
							...
							{!spanNamesLoading && availableSpanNames.length === 0 && (
								<p className="text-xs text-muted-foreground italic">
									No span names found in{" "}
									{selectedGraph ? `graph "${selectedGraph}"` : "any graph"}.
						...
						<div className="space-y-2">
							...
							{attributes.length === 0 && (
								<p className="text-xs text-muted-foreground italic">
									No attribute filters added. Click "Add Attribute" to filter by
									span attributes.
								</p>
							)}
						</div>
