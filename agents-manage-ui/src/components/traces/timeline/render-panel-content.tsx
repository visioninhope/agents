			...
			return (
				<>
					<Section>
						<Info
							label="Model"
							value={<ModelBadge model={a.aiModel || "Unknown"} />}
						/>
						<Info
							label="Input Tokens"
							value={a.inputTokens?.toLocaleString() || "0"}
						/>
						<Info
							label="Output Tokens"
							value={a.outputTokens?.toLocaleString() || "0"}
						/>
				...
				<>
					<Section>
						<LabeledBlock label="Message Content">
							<Bubble className="">
								{a.messageContent || "Message content not available"}
							</Bubble>
						</LabeledBlock>
						<Info
							label="Message Length"
							value={`${a.messageContent?.length || 0} characters`}
						/>
						<StatusBadge status={a.status} />
						<Info label="Timestamp" value={formatDateTime(a.timestamp)} />
					</Section>
				...
				<>
					<Section>
						...
						<Info
							...
							value={formatDateTime(a.timestamp)}
						/>
						<Info
							label="Message ID"
							value={
			...
			return (
				<>
					<Section>
						...
						{a.contextTrigger && (
							<Info label="Trigger" value={a.contextTrigger} />
						)}
						<StatusBadge status={a.status} />
			...
			return (
				<>
					<Section>
						...
						<Info
							label="Result"
							value={a.toolResult || "No result available"}
						/>
						<Info label="Timestamp" value={formatDateTime(a.timestamp)} />
					</Section>
					<Divider />
				...
				<>
					<Section>
						...
						<LabeledBlock label="To Agent">
							<Badge variant="code" className="">
								{a.transferToAgentId ||
									a.toolName?.replace("transfer_to_", "") ||
									"Unknown Target"}
							</Badge>
						</LabeledBlock>
						<Info
				...
				<>
					<Section>
						...
						<LabeledBlock label="Purpose">
							<Bubble className="b">
								{a.toolPurpose || "No purpose information available"}
							</Bubble>
						</LabeledBlock>
			...
			return (
				<>
					<Section>
						<Info
							label="Model"
							value={<ModelBadge model={a.aiStreamTextModel || "Unknown"} />}
						/>
						<Info
							label="Operation ID"
							value={
