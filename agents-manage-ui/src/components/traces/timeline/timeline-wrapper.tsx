...
export function TimelineWrapper({
	...
	const [collapsedAiMessages, setCollapsedAiMessages] = useState<Set<string>>(
		new Set(),
	);
	...
	const findSpanById = (id?: string) =>
		conversation?.allSpanAttributes?.find(
			(s: NonNullable<ConversationDetail["allSpanAttributes"]>[number]) =>
				s.spanId === id,
		);

	const determinePanelType = (
		a: ActivityItem,
	): Exclude<PanelType, "mcp_tool_error"> => {
		if (
			a.type === ACTIVITY_TYPES.TOOL_CALL &&
			a.toolType === TOOL_TYPES.TRANSFER
		)
			return "transfer";
