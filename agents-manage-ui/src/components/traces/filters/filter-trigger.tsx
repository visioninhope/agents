...
export function FilterTriggerComponent({
	...
	const firstSelectedOptionValue =
		countMultipleFilterValues && multipleCheckboxValues[0];
	const firstSelectedOptionLabel =
		options.find((option) => option.value === firstSelectedOptionValue)
			?.label || firstSelectedOptionValue;
				...
				>
					{hideFilterLabelWhenSelected && isShowSelectedOptions ? null : (
						<div className="flex items-center gap-2">
							{Icon && (
								<Icon className="h-4 w-4 text-gray-400 dark:text-white/50" />
							)}
							{filterLabel}
						</div>
					)}
