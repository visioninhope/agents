...
function createFormSchema(
	formConfig: NonNullable<ReturnType<typeof getFormConfig>>,
) {
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-muted-foreground">
							No configuration form is available for {provider.auth_mode}{" "}
							authentication mode.
						</p>
					</div>
				</div>
			...
			return (
				<div key={field.key} className="space-y-2">
					<GenericTextarea
						...
						className={
							isPrivateKey ? "font-mono text-sm min-h-[120px]" : "min-h-[80px]"
						}
					/>
					{field.helpText && (
				...
				<div className="flex items-center gap-3">
					...
					<div>
						<h1 className="text-lg font-medium">
							Setup {provider.display_name || provider.name}
						</h1>
						<p className="text-muted-foreground">
