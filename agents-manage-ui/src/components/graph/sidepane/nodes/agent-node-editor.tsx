...
export function AgentNodeEditor({
	...
	const selectedArtifactComponents =
		selectedNode.data?.artifactComponents || [];

	// Get project and graph data for inheritance indicators
	const { project } = useProjectData();
	const metadata = useGraphStore((state) => state.metadata);

	const { updatePath, updateNestedPath, getFieldError, setFieldRef } =
		useNodeEditor({
			...
			<div className="space-y-4">
				...
				<div className="space-y-2">
					...
					<p className="text-xs text-muted-foreground">
						Maximum number of execution steps for this agent (defaults to 50 if
						not set)
					</p>
				</div>
				...
				<div className="text-xs text-muted-foreground p-3 bg-blue-50 dark:bg-blue-950/20 rounded-md border border-blue-200 dark:border-blue-800">
					...
					<ul className="space-y-1 text-blue-800 dark:text-blue-200">
						<li>
							• <strong>stepCountIs</strong>: Project → Agent only (agent-level
							execution limit)
						</li>
						<li>
						...
						<li>
							• <strong>Agent scope</strong>: This limit applies only to this
							specific agent's execution steps
						</li>
						<li>
							• <strong>Independent from transfers</strong>: Steps are counted
							per agent, transfers are counted per conversation
						</li>
					</ul>
				</div>
			</div>
