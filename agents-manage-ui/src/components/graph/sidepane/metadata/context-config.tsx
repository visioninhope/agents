...
import type {
	ContextConfig,
	GraphMetadata,
} from "../../configuration/graph-types";
...
export function ContextConfigForm({
	...
	const { id, name, description, contextVariables, requestContextSchema } =
		contextConfig;
		...
		<div className="space-y-8">
			<div className="space-y-2">
				...
				<p className="text-sm text-muted-foreground">
					Configure dynamic context for this graph.
				</p>
			</div>
			<div className="flex flex-col space-y-8">
				<div className="space-y-2">
					...
					/>
					<p className="text-sm text-muted-foreground">
						Choose a unique identifier for this configuration. Using an existing
						id will replace that configuration.
					</p>
				</div>
				<div className="space-y-2">
				...
				<ExpandableJsonEditor
					...
					onChange={(value) =>
						updateContextConfig("requestContextSchema", value)
					}
					placeholder="{}"
					className=""
				/>
