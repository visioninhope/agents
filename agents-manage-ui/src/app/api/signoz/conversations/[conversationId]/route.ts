	...
	try {
		...
		if (axios.isAxiosError(e)) {
			...
			if (e.response?.status === 401 || e.response?.status === 403) {
				throw new Error(
					`SigNoz authentication failed: ${e.response.statusText}`,
				);
			}
		...
		throw new Error(
			`SigNoz query failed: ${e instanceof Error ? e.message : "Unknown error"}`,
		);
	}
}
...
function parseList(resp: SigNozResp, name: string): SigNozListItem[] {
	const list =
		resp?.data?.result?.find((r) => r?.queryName === name)?.list ?? [];
...
export async function GET(
	...
	try {
		...
		for (const span of toolCallSpans) {
			...
			const delegationFromAgentId = getString(
				span,
				SPAN_KEYS.DELEGATION_FROM_AGENT_ID,
				"",
			);
			const delegationToAgentId = getString(
				span,
				SPAN_KEYS.DELEGATION_TO_AGENT_ID,
				"",
			);
			const transferFromAgentId = getString(
				span,
				SPAN_KEYS.TRANSFER_FROM_AGENT_ID,
				"",
			);
			const transferToAgentId = getString(
				span,
				SPAN_KEYS.TRANSFER_TO_AGENT_ID,
				"",
			);
			...
			if (name === TOOL_NAMES.SAVE_TOOL_RESULT) {
				...
				const functionId = getString(
					span,
					SPAN_KEYS.AI_TELEMETRY_FUNCTION_ID,
					"",
				);
				...
				try {
					...
					saveFields = {
						...
						saveArtifactType:
							parsed.artifactType || parsedArgs.artifactType || undefined,
						saveArtifactName:
							firstArtifact?.name || parsedArgs.name || undefined,
			...
			activities.push({
				...
				agentName: getString(
					span,
					SPAN_KEYS.AI_AGENT_NAME,
					ACTIVITY_NAMES.UNKNOWN_AGENT,
				),
				result: hasError
					? `Tool call failed (${durMs.toFixed(2)}ms)`
					: `${durMs.toFixed(2)}ms`,
		...
		for (const span of contextResolutionSpans) {
			...
			activities.push({
				...
				contextConfigId:
					getString(span, SPAN_KEYS.CONTEXT_CONFIG_ID, "") || undefined,
				contextAgentGraphId:
					getString(span, SPAN_KEYS.CONTEXT_AGENT_GRAPH_ID, "") || undefined,
		...
		for (const span of aiAssistantSpans) {
			...
			activities.push({
				...
				aiResponseTimestamp:
					getString(span, SPAN_KEYS.AI_RESPONSE_TIMESTAMP, "") || undefined,
			});
		}
		...
		for (const span of aiGenerationSpans) {
			...
			activities.push({
				...
				agentName:
					getString(span, SPAN_KEYS.AI_TELEMETRY_FUNCTION_ID, "") || undefined,
				result: hasError
		...
		for (const span of aiStreamingSpans) {
			...
			activities.push({
				...
				aiStreamTextModel: getString(
					span,
					SPAN_KEYS.AI_RESPONSE_MODEL,
					"Unknown Model",
				),
				aiStreamTextOperationId:
					getString(span, SPAN_KEYS.AI_OPERATION_ID, "") || undefined,
				inputTokens: getNumber(span, SPAN_KEYS.GEN_AI_USAGE_INPUT_TOKENS, 0),
				outputTokens: getNumber(span, SPAN_KEYS.GEN_AI_USAGE_OUTPUT_TOKENS, 0),
			});
		}
		...
		const allSpanTimes = durationSpans.map((s) =>
			new Date(s.timestamp).getTime(),
		);
		const operationStartTime =
			allSpanTimes.length > 0 ? Math.min(...allSpanTimes) : null;
		const operationEndTime =
			allSpanTimes.length > 0 ? Math.max(...allSpanTimes) : null;
		...
		const firstUser = activities.find(
			(a) => a.type === ACTIVITY_TYPES.USER_MESSAGE,
		);
		...
		const conversation = {
			...
			totalToolCalls: activities.filter(
				(a) => a.type === ACTIVITY_TYPES.TOOL_CALL,
			).length,
