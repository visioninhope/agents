...
export class AgentGraph implements GraphInterface {
	...
	private async toFullGraphDefinition(): Promise<FullGraphDefinition> {
		...
		for (const agent of this.agents) {
			if (this.isInternalAgent(agent)) {
				...
				agentsObject[internalAgent.getId()] = {
					...
					description:
						internalAgent.config.description ||
					...
					artifactComponents:
						artifactComponents.length > 0 ? artifactComponents : undefined,
					type: "internal",
				};
			} else {
		...
		return {
			...
			dataComponents:
				Object.keys(dataComponentsObject).length > 0
					? dataComponentsObject
					: undefined,
			artifactComponents:
				Object.keys(artifactComponentsObject).length > 0
	...
	removeAgent(id: string): boolean {
		...
		if (agentToRemove) {
			...
			this.agents = this.agents.filter(
				(agent) => agent.getId() !== agentToRemove.getId(),
			);
	...
	isInternalAgent(agent: AllAgentInterface): agent is AgentInterface {
		...
		return (
			"getTransfers" in agent &&
			typeof (agent as any).getTransfers === "function"
		);
	}
	 ...
	 */
	private async getProjectModelDefaults(): Promise<
	...
	private propagateModelSettingsToAgent(agent: AgentInterface): void {
		if (this.models) {
			...
			if (
				!agent.config.models.structuredOutput &&
			...
			if (this.isInternalAgent(agent)) {
				...
				for (const delegate of delegates) {
					...
					if (delegate instanceof ExternalAgent) {
						allRelationPromises.push(
							this.createExternalAgentRelation(agent, delegate, "delegate"),
						);
					} else {
	...
	private async createExternalAgents(): Promise<void> {
		const externalAgents = this.agents.filter((agent) =>
			this.isExternalAgent(agent),
		);
