      ...
      if (this.isInternalAgent(agent)) {
        ...
        for (const [toolName, toolInstance] of Object.entries(agentTools)) {
          if (toolInstance && typeof toolInstance === 'object') {
            const toolId = (toolInstance as any).getId?.() || (toolInstance as any).id || toolName;
            tools.push(toolId);
          }
        }
      ...
      for (const [toolName, toolInstance] of Object.entries(agentTools)) {
        if (toolInstance && typeof toolInstance === 'object') {
          ...
          if (!toolsObject[toolId]) {
            ...
            const toolData: any = {
              id: toolId,
              name: (toolInstance as any).config?.name || (toolInstance as any).name || toolName,
              config: toolConfig,
              status:
                (toolInstance as any).getStatus?.() || (toolInstance as any).status || 'unknown',
            };
            ...
            if ((toolInstance as any).getCredentialReferenceId?.()) {
              toolData.credentialReferenceId = (toolInstance as any).getCredentialReferenceId();
            }
