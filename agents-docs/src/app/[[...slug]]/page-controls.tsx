...
export function PageControls({
  ...
  const openInChatGPT = () => {
    const uri = `https://chat.openai.com/?model=gpt-4&q=${encodeURIComponent(
      askPrompt
    )}`;
    setOpen(false);
...
function Item({ title, description, icon, className, ...props }: ItemProps) {
  return (
    <span
      ...
      {...props}
    >
      <i className="rounded-lg p-1.5 m-px border text-fd-muted-foreground">
        {icon}
      </i>
      <div className="flex-1 text-start">
        <p className="text-[15px] font-medium md:text-sm whitespace-nowrap">
          {title}
        </p>
        {description ? (
