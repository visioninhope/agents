...
## Creating a Custom Evaluator for the Weather Graph

Let's walk through creating a custom evaluator specifically for the Weather Graph. This evaluator will assess whether the agent correctly provides a 24-hour temperature forecast for a given location.

### Understanding the Weather Graph Structure

The Weather Graph consists of three agents:
- **Weather Assistant**: Routes between geocoder and weather forecast agents
- **Geocoder Agent**: Converts locations/addresses into coordinates using `geocoding_tool` tool
- **Weather Forecaster**: Takes coordinates and returns weather forecasts using `get_weather_forecast` tool
...
The evaluator will score traces based on whether the weather graph output contains:

**Score 1 (Positive):**
- ✅ Specific temperature values (e.g., "72°F", "22°C")
- ✅ Time-based forecast (hourly or daily breakdown)
- ✅ Location confirmation
- ✅ Clear forecast duration (24 hours)

**Score 0 (Negative):**
- ❌ Generic responses without specific temperatures
- ❌ Error messages or failed tool calls
- ❌ Incomplete location processing
- ❌ Missing time-based breakdown

### Advanced Configuration Tips

**For Production Use:**
- **Reduce sampling** to 10-25% to control evaluation costs
- **Add trace filters** to focus on specific user interactions
- **Monitor evaluation metrics** regularly in the Langfuse dashboard

**For Development:**
- **Use 100% sampling** during testing phases
- **Create multiple evaluators** for different aspects (accuracy, completeness, format)
- **Set up alerts** for evaluation score drops

**Common Variable Mappings for Weather Graph:**
- `{{input}}` → User's original weather query
- `{{output}}` → Final weather forecast response
- `{{location}}` → Extracted location from geocoder agent
- `{{coordinates}}` → Latitude/longitude from geocoder
- `{{forecast_data}}` → Raw weather data from forecast tool

This custom evaluator will help you monitor the quality and accuracy of your Weather Graph's responses, ensuring users receive proper 24-hour temperature forecasts for their requested locations.
