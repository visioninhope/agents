...
Every agent needs a unique identifier and a clear prompt that define its behavior:

```typescript
import { agent } from "./builder/index.js";
...
| `tools` | object | No | Custom and MCP tools that the agent can use. See [Tools](/typescript-sdk/tools) for details |
...
| `artifactComponents` | array | No | Components for handling tool or agent outputs. See [Artifact Components](/typescript-sdk/artifacts) for details |
| `canTransferTo` | function | No | Function returning array of agents this agent can transfer to. See [Transfer Relationships](/typescript-sdk/sub-agents#transfer-relationships) for details |
| `canDelegateTo` | function | No | Function returning array of agents this agent can delegate to. See [Delegation Relationships](/typescript-sdk/sub-agents#delegation-relationships) for details |
...
Each model setting can include `providerOptions` with provider-specific settings:
