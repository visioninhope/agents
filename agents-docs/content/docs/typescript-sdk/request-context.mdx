...
const personalAssistant = agent({
  ...
  prompt: `You are a helpful assistant for ${personalGraphContext.toTemplate('user.name')} from ${personalGraphContext.toTemplate('headers.org_name')}.

  User ID: ${personalGraphContext.toTemplate('headers.user_id')}
