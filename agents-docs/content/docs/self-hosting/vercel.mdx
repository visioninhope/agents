...
Follow these instructions on getting started with [SigNoz Cloud](/quick-start/observability#option-1-signoz-cloud-setup).
```
SIGNOZ_URL=
SIGNOZ_API_KEY=
...
Follow these instructions on getting started with [Nango Cloud](/quick-start/credentials#option-1-nango-cloud-setup).
```
NANGO_SECRET_KEY=
```
...
[Follow these steps in the docs](/quick-start/start-development) with the `npx @inkeep/create-agents` CLI command to create a project.
